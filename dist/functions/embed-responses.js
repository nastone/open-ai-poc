...
function createEmbeddingBatches(objects, maxTokens = 8191 * 0.8) {
    ...
    for (const obj of objects) {
        ...
        if (currentBatchTokens + tokenCount > maxTokens) {
            console.log(`Batch ${batches.length} created with an estimated token count of ${currentBatchTokens} `);
            // Start a new batch
...
export const embedResponses = async () => {
    ...
    for (const batch of batches) {
        ...
        console.log(`Embeddings created for batch ${processedBatchCount + 1}, used ${usage.total_tokens} tokens.`);
        await Promise.all(batch.map((response, i) => {
